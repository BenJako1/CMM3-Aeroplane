# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 02:58:17 2023
...
#------------------------------------------------------------------------------
# Class for handling the initial and user inputted trim conditons
...
class Trim:
    def __init__(self, trimVelocity, trimGamma):
        ...
        # Solve for angle of attach using Newton Raphson root finder
...
# Takes simulation data as input
...
class Visualise():
    # Plotting aircraft parameter response
    def Display_Sim(self, Data, initialAltitude):
        # Extracting data and defining components
        self.t = Data.t
        self.q = Data.y[0]
        self.theta = np.rad2deg(Data.y[1])
        self.ub = Data.y[2]
        ...
        # Output the plot
        plt.show()
        # Creating a plot for each extracted parameter
        fig, ax = plt.subplots(3, 2, figsize=(12, 10))
...
class Simulation(Visualise):
    def __init__(self, trimVelocity, trimGamma, t_end):
        ...
        self.Display_Sim(y, initialAltitude)
...
#-----------------------------------------------------------------------------------
# B1 Calculating trim conditions for various values of velocity and path angle
#------------------------------------------------------------------------------

class B1(Visualise):
    def __init__(self, V_min, V_max, gamma_min, gamma_max, V_step, gamma_step):
        # Define the ranges for Veloctiy V and path angle gamma
        self.V_min = V_min
        self.V_max = V_max
        ...
        #Iterating over V and path angle values to produce charts with multiple velocity and path angle trim conditions
        for i, V in enumerate(self.V_values):
            for j, gamma in enumerate(self.gamma_values):
                ...
                # Store thrust and elevato angle values from the trim condition
...
class B2(Visualise):
    def __init__(self, trimVelocity, trimGamma, t_end, initialAltitude, maxAltitude, pitchTime, climbVelocity, climbGamma, climbTimeGuess = 0, climbStep = 0.5):
        ...
        # Restricts simulation until secified altitude is reached
...
# A3 
#-----------------------------------------------------------------------------------------------------------
    sim = Simulation(100, 0, 2000, 1000, [(100.0, -0.002, 0.0), (300.0, 0.002, 0.0)])
    sim.Display_Sim(sim.data, initialAltitude)
#-----------------------------------------------------------------------------------------------------------
